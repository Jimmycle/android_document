...
2. 从Android Driver层：Binder还可以理解为一种虚拟的物理设备，设备驱动是/dev/binder；
...
![binder_arch](https://gityuan.com/images/binder/java_binder/java_binder.jpg)

Binder在Native层有一套完整的binder通信的C/S架构(图中的蓝色)，BpBinder作为客户端，BBinder作为服务端。基于naive层的Binder框架，Java也有一套镜像功能的binder C/S架构，通过JNI技术，与native层的binder对应，Java层的binder功能最终都是交给native的binder来完成。

### Binder进程与线程

![binder_proc_relation](https://gityuan.com/images/binder/summary/binder_proc_relation.png)
...
![binder_transaction](https://gityuan.com/images/binder/summary/binder_transaction.png)
